...
def model_init(model, mode='orthogonal'):
    ...
    print(f"Proceed right before model.apply(..) at model_init, with params: {model_init_dict} \nand mode:{mode}")
    model.apply(model_init_dict[mode])
    print(f"We didnt get to after model.apply()")
...
def ortho_init(model):
    ...
    for m in model.modules():
        if isinstance(m, (nn.Conv2d, nn.Conv3d, nn.Linear)):
            ...
            print(f'What is m? {m}')
    #print("Orthogonal initialization is done.")
